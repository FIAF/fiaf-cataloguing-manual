...
def build_frontmatter(title: str, weight: Optional[int]) -> str:
...
def main() -> int:
    ...
    for src in sources:
        ...
        weight_match = re.match(r"^(\d+)_", folder)
        folder_weight = int(weight_match.group(1)) + 1 if weight_match else None
        ...
        # Build folder index from preamble + first section block (if any).
        if blocks:
            first_block = blocks[0]
            index_raw = "\n\n".join([preamble, first_block]).strip()
            fm = build_frontmatter(chapter_title, folder_weight)
            index_dest = ROOT / "static/content/docs" / folder / "_index.md"
            ...
            if chapter_label and chapter_label in label_map:
                # Ensure chapter label resolves to the folder page.
                label_map[chapter_label].anchor = None

            # Remaining sections become subpages.
            ...
            for i, block in enumerate(blocks[1:], start=1):
                ...
                if count > 1:
                    slug = f"{slug}-{count}"
                dest = ROOT / "static/content/docs" / folder / slug / "index.md"
                fm = build_frontmatter(title, i)
            ...
            # No sections; dump all content into folder index.
            index_dest = ROOT / "static/content/docs" / folder / "_index.md"
            title = folder.replace("_", " ").title()
            fm = build_frontmatter(title, folder_weight)
