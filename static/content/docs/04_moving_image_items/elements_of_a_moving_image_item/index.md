...
weight: 1
---
<a id="sec-elements_of_a_moving_image_item"></a>
### Elements of a Moving Image Item
...
See “Shallow hierarchy model: 2 levels” in [Elements of description across Works, Variants, Manifestations, and Items](/docs/00_preliminary/core_elements_of_description/#sec-elements_of_description)).
...
This chapter contains Item-specific physical/digital description elements beginning at Section 3.1.5 (see [Item Specifics/Extent (e.g. physical/Digital description)](/docs/04_moving_image_items/elements_of_a_moving_image_item/#sec-item_specifics_extent).
...
#### Identifier
Create an unambiguous numerical or alphanumerical reference to the moving image Item, such as a call number, barcode, shelf mark or similar, to uniquely identify the copy.[^fn3] This may be in addition to separate Acquisition and Accession number(s) or identifier(s).
...
If multiple titles are recorded, where allowable, associate a “Title Type” to a title for differentiation between the various types of titles (see [Title Types](/docs/09_appendix_01/title_types/#sec-title_types)).
...
For Items where the only title information is found on a can or leader, use an Acquisition Title Type(s) (see [Alternative title types](/docs/09_appendix_01/title_types/#sec-alternative_title_types)) or descriptive words such as “Title on can” or “Title on leader.”

<a id="sec-holding_institution"></a>
#### Holding Institution[^fn1]
...
A suggested list, which is open and not exhaustive, can be found in [Item Element Type](/docs/12_appendix_04/manifestation_item_physical_description/#sec-appendix_item_element_type).
...
Often there will be physically separate Items, for example image, track, music, different colour bases, etc., which are all essential parts of a single moving image.[^fn4] Descriptive terminology covering all areas of physical description and attributes should be established in controlled lists of terms, to be applied in the relevant categories.
...
Further data relating to the condition, preservation, location, and, acquisition, accessioning, and source of the Item are also recommended elements for the Item (see [Access Conditions](/docs/04_moving_image_items/elements_of_a_moving_image_item/#sec-access_conditions)).
...
<a id="sec-item_general_carrier_type"></a>
...
A suggested list can be found in [Manifestation/Item General Carrier Type](/docs/12_appendix_04/manifestation_item_physical_description/#sec-manifestation_item_general_carrier_type).
...
<a id="sec-item_specific_carrier_type"></a>
...
A suggested list, which is open and not exhaustive, can be found in [Item Preservation/Access status](/docs/12_appendix_04/manifestation_item_physical_description/#sec-item_preservation_access_status).

<a id="sec-sound"></a>
##### Sound
Technical specifications relating to the fixation of sound in a moving image Manifestation/Item (see [Sound Characteristics of a Manifestation](/docs/03_moving_image_manifestations/elements_of_a_manifestation/#sec-sound_characteristics_of_a_manifestation)).
...
##### Sound System
See also [Sound Characteristics of a Manifestation](/docs/03_moving_image_manifestations/elements_of_a_manifestation/#sec-sound_characteristics_of_a_manifestation)
...
<a id="sec-colour"></a>
##### Colour
For full instructions, see [Colour Characteristics of a Manifestation](/docs/03_moving_image_manifestations/elements_of_a_manifestation/#sec-colour_characteristics_of_a_manifestation).

The presence of colour(s), tone(s), etc. in an Item.[^fn5]

Record only if this information is not captured at the Manifestation level or if required at the Item level by the system in use.

<a id="sec-unit_number"></a>
##### Unit Number
For full instructions see [Logical Extent of a Manifestation](/docs/03_moving_image_manifestations/elements_of_a_manifestation/#sec-logical_extent_of_a_manifestation)
...
This footage represents actual length, rather than the “ideal” length, which is recorded for Manifestations (see [Physical Extent of a Manifestation](/docs/03_moving_image_manifestations/elements_of_a_manifestation/#sec-physical_extent_of_a_manifestation)).
...
For full instructions, see [Projection Characteristics of a Manifestation{](/docs/03_moving_image_manifestations/elements_of_a_manifestation/#sec-projection_characteristics_of_a_manifestation)
...
This duration represents actual temporal extent, rather than the “ideal” temporal extent, which is recorded for Manifestations (see [Duration of a Manifestation](/docs/03_moving_image_manifestations/elements_of_a_manifestation/#sec-duration_of_a_manifestation)).
...
Information related to the frame rate used during a digitisation process is added to Transfer Speed (see [Transfer Speed](/docs/04_moving_image_items/elements_of_a_moving_image_item/#sec-transfer_speed)).
...
A suggested list, which is open and not exhaustive, can be found in [Item Stock](/docs/12_appendix_04/manifestation_item_physical_description/#sec-item_stock).
...
##### Transfer Speed
If the Transfer Speed is the same as the [Frame Rate](/docs/04_moving_image_items/elements_of_a_moving_image_item/#sec-frame_rate), these values should be the same.
...
Record these actions as an Event (see [Events](/docs/04_moving_image_items/relationships_of_an_item/#sec-items_events)), with the person or entity performing the action as an Agent (see [sec:items_agents](#sec-items_agents)).
...
Selection should be made from a controlled list of terms, for example, “brittle”, “buckled”, “tears,” etc. A suggested list, which is open and not exhaustive, can be found in [Item Copy Condition Base/Emulsion – Film and Video](/docs/12_appendix_04/manifestation_item_condition_preservation_and_restoration/#sec-item_copy_condition_base_emulsion_film_and_video).

Capture the date on which the condition of the Item was recorded (using ISO standard[^fn6]).
...
If possible, use the Item Identifier and Identifier Type fields to note an Item’s location (see [Identifier](/docs/04_moving_image_items/elements_of_a_moving_image_item/#sec-item_identifier)).
...
#### Notes for Items
Notes for Items are an annotation providing additional information relating specifically to Item attributes and relationships.[^fn7] See [Cataloguer’s Notes](/docs/10_appendix_02/).

[^fn1]: Based on EN 15907, Holding institution
[^fn2]: Based on EN 15907, Instantiation type
[^fn3]: EN 15907, “Inventory number,” p. 12
[^fn4]: See The FIAF Cataloguing Rules for Film Archives (1991). 5. Physical Description. Introduction
[^fn5]: RDA 7.17.3 Colour of Moving Image
[^fn6]: ISO 8601
[^fn7]: Based on RDA 2.20.1.Basic Instructions on Making Notes on Manifestations or Items
