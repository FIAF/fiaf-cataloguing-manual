...
def render():
    ...
    with open(pathlib.Path.cwd() / 'figure_17.json') as data:
        data = json.load(data)
    ...
    test_path = pathlib.Path.cwd() / 'figure_17.png'
    ...
    for a in data['arrows']:
        ...
        if a['type'] == 'multi_v':
            ...
            cv2.line(canvas, (ax,ay), (bx,ay), (0,0,0), 1)
            cv2.line(canvas, (bx,by), (bx,ay), (0,0,0), 1)
            cv2.arrowedLine(canvas, (ax+45,ay), (ax,ay), (0,0,0), 1)
            cv2.arrowedLine(canvas, (bx,by-45), (bx,by), (0,0,0), 1)
