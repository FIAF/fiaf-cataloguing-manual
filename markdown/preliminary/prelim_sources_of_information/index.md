...
e) accompanying material or a container issued as part of the resource itself[^5]
...
The number of websites stating the same “fact” is not a safe indicator. Websites copy off each other and it is easy for an erroneous fact to be perpetuated across multiple websites[^9]. The cataloguer needs to assess, balance, and judge accuracy, e.g. IMDB and Wikipedia can be good for information, but they also allow submissions and changes from the public so it is not necessarily always accurate.

The same cautious approach should be taken when using AI for research or information purposes relating to moving images, e.g. as Google AI view itself makes the point that "AI research answers can be very different or inconsistent, even when you ask the exact same question multiple times or use different tools." It can be a useful tool for finding out facts about moving images (and it usually cites its sources), e.g. finding out the year of release, credits, etc. AI heavily uses the worldwide web and resources on it as its source and it can also be inconsistent with results. Asking the same question in a slightly different way may bring back conflicting answers. It is best to be precise rather than very general in how questions are phrased, but conversely not too complex as that can also lead to divergent answers or results.
...
[^3]: RDA 2.20.2.3 Title Source
[^4]: Adapted from RDA 2.2.2.3 Resources Consisting of Moving Images
[^5]: Based on RDA 2.2.4 Other Sources of Information
[^6]: RDA 2.2.4 Other Sources of Information
